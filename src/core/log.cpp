...
void Log::log_with_date_time(const string &message, Level level) {
    ...
    // IMPORTANT: std::locale takes ownership of the facet pointer passed to its ctor.
    // Passing the address of a static facet will cause an invalid free when the locale is destroyed.
    static const std::locale time_locale(std::locale::classic(), new time_facet("[%Y-%m-%d %H:%M:%S] "));

    ostringstream stream;
    stream.imbue(time_locale);
