...
import 'package:trojan_pro_client/features/diagnostics/domain/routing_recovery_record.dart';
...
void main() {
  test('diagnostics export includes routing recovery evidence', () async {
    ...
    final diagnostics = DiagnosticsExportService(
      ...
      routingRecoveryRecords: <RoutingRecoveryRecord>[
        RoutingRecoveryRecord(
          operationId: 'connect-17',
          profileId: 'sample-hk-1',
          rollbackReason: 'mini smoke failed on rule-direct',
          safeModeActivated: true,
          quarantined: false,
          timestamp: DateTime.parse('2026-04-16T10:00:00.000Z'),
        ),
      ],
    );
    ...
    expect(payload['routingRecoveryEvidence'], isNotNull);
    final records = payload['routingRecoveryEvidence'] as List<dynamic>;
    expect(records, hasLength(1));

    final first = records.first as Map<String, dynamic>;
    expect(first['operationId'], 'connect-17');
    expect(first['profileId'], 'sample-hk-1');
    expect(first['rollbackReason'], contains('mini smoke failed'));
    expect(first['safeModeActivated'], isTrue);
    expect(first['quarantined'], isFalse);
  });
}
